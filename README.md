...
0. **Train the arena detector at proper scale** (the one thing blocking "accurate"). `scripts/kaggle_train.py` runs this on Kaggle's free GPU: `package` bundles the code, sprites, 2026 backgrounds and base weights; `push` uploads and starts a kernel; `fetch` retrieves the weights. `scripts/simulate_kaggle.py` runs the same kernel locally against a read-only mirror first — it exists because a failed kernel returns a bare `ERROR` and burns a queue slot. Platform quirks worth knowing, each found the hard way: Kaggle kernels have **no internet** (dependencies must be vendored, not pip-installed), the dataset mounts at `/kaggle/input/datasets/<owner>/<slug>/`, a kernel pushed before `dataset_status == "ready"` silently runs against the **previous** dataset version, and the mount is read-only *including directories* — so code that writes next to its own source must be copied to a writable dir first, with permissions restored (`copytree` preserves the read-only bits). Everything upstream is built and verified: 2026 backgrounds, new-class sprite fetching, registration into spare head slots, and a generator that auto-labels. What remains is a real training run — a few thousand generated images, two-phase freeze/unfreeze, with replay of the original data — on a rented or free cloud GPU rather than MPS. Measure with `scripts/val_detector.py` against the 0.863 mAP50 baseline; only ship weights that beat it.
